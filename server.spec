...
# Include user_data (configs, cache, etc. – NO MODELS)
...
if os.path.exists(user_data_path):
    for root, dirs, files in os.walk(user_data_path):
        ...
        for file in files:
            if not file.endswith(('.pyc', '.pyo')):
                # Skip models folder to keep build light
                if "models" not in root:
                    file_path = os.path.join(root, file)
                    rel_path = os.path.relpath(root, script_dir)
                    datas.append((file_path, rel_path))

# Ensure user_data/models exists, even if empty
models_path = os.path.join(user_data_path, "models")
if os.path.exists(models_path):
    ...
    # Add an empty placeholder file so the folder is created at runtime
    placeholder = os.path.join(script_dir, "user_data", "models", ".placeholder")
...
# === ASSETS: css, js, extensions ===
asset_dirs = ["css", "js", "extensions"]
...
# Collect submodules
submodule_packages = ['pytz', 'transformers', 'gradio', 'uvicorn', 'fastapi', 'llama_cpp_binaries']
...
a.hiddenimports = list(set(a.hiddenimports))  # deduplicate
