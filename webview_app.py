...
try:
    ...
    print("ERROR: pywebview not found. Please install it with:")
    print("pip install pywebview")
    print("")
    print("Also install pyyaml if not present:")
...
def create_interface():
    ...
    # Store the Gradio app for webview
    gradio_app = shared.gradio['interface']

    # Launch the interface in webview-compatible mode
    shared.gradio['interface'].queue()

    # Important: prevent_thread_lock=False to let Gradio own its event loop, and run it in its own thread
    def _launch():
        global server_url
        with OpenMonkeyPatch():
            shared.gradio['interface'].launch(
                ...
                server_port=shared.args.listen_port or 7860,
                inbrowser=False,
                auth=auth or None,
    ...
    raise RuntimeError("Gradio server did not start in time")


def start_webview():
    """Start the webview application"""
    try:
        # Create the Gradio interface and get the URL
        url = create_interface()
        logger.info(f"Gradio server started at: {url}")
...
if __name__ == "__main__":
    ...
    logger.info("Starting Text Generation Web UI in WebView")
